...
TEST_F(GraphTest, LoadGraphFromFile_Success) {
  ...
  bool result = graph.LoadGraphFromFile(filename);
...
TEST_F(GraphTest, LoadGraphFromFile_CheckNormalDimenssion) {
  ...
  auto result = graph.GetGraph().GetRows();
...
TEST_F(GraphTest, LoadGraphFromFile_CheckData) {
  ...
  graph.LoadGraphFromFile("../materials/examples/graph_1.txt");
  s21::matrix<int> source_graph = graph.GetGraph();
  for (size_t row = 0; row < graph.GetGraph().GetRows(); ++row) {
    for (size_t col = 0; col < graph.GetGraph().GetCols(); ++col) {
      EXPECT_TRUE(source_graph[row][col] ==
                  data[row * graph.GetGraph().GetCols() + col])
