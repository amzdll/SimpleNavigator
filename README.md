...
3. [Chapter III](#chapter-iii) \
    ...
    3.2. [Part 2](#part-2-finding-the-shortest-paths-in-a-graph_test_)  
...
Leonard Euler (1707-1783) is the founder of the graph_test_ theory. The origin of this theory can be traced back to the correspondence of the great scholar. Here is a translation of the Latin text, which is cited from a letter Euler sent from St. Petersburg on March 13, 1736, to the Italian mathematician and engineer Marinoni:
...
You can see how Königsberg bridges are situated in the following picture, where the vertices of the graph_test_ correspond to a certain part of the city, and the edges correspond to bridges over the river, where A is an island, and B, C and D are parts of the continent, separated from each other by arms of the river.

![konigsberg-graph_test_](misc/images/konigsberg-graph_test_.png)
...
1) The number of odd vertices (vertices to which an odd number of edges lead) of a graph_test_ must be even.
There cannot be a graph_test_ that has an odd number of odd vertices.
2) If all vertices of a graph_test_ are even (vertices that are led to by an even number of edges), you can draw the graph_test_ without taking your pencil off the paper, and you can start from any vertex of the graph_test_ and end it at the same vertex.
3) A graph_test_ with more than two odd vertices cannot be drawn with a single stroke.

The Königsberg bridge graph_test_ had four odd vertices, hence, it is impossible to traverse all the bridges without passing over any of them twice.

Until the beginning of the 20th century, graph_test_ theory was developing mainly in the formulation of new theorems based on the results of solving various 'puzzle problems'. Graph theory was seriously developed due to the emergence of large-scale mass production and general breakthroughs in science and technology in the first half of the 20th century.  

### Main terms

A **Graph** has a finite set of vertices, and a set of edges. Each edge has two points from the set of graph_test_ vertices that form the edge points.

**Types of graphs based on the type of edges:**
* *undirected* - a graph_test_ in which movement between vertices connected by an edge is possible in any direction

![not-orient-graph_test_](misc/images/not-orient-graph_test_.png)

* *directed* - a graph_test_, the edges of which have a direction. Directed edges are also called arcs. Moving from one vertex to another is possible only by arcs of corresponding direction

![orient-graph_test_](misc/images/orient-graph_test_.png)

If besides the edge between two vertices, the weight of the edge is also given, then such a graph_test_ is called ***weighted***.

![weighted-orient-graph_test_](misc/images/weighted-orient-graph_test_.png)


**Types of graphs based on the number of edges:**
* a *null graph_test_* is a graph_test_ in which there are no edges

![null-graph_test_](misc/images/null-graph_test_.png)

* *incomplete* - the graph_test_ has edges, but not from every vertex, there is an edge to every other vertex

![not-full-graph_test_](misc/images/not-full-graph_test_.png)

* *complete* - the graph_test_ has an edge from each vertex to any other vertex

![full-graph_test_](misc/images/full-graph_test_.png)


**Types of graphs based on node reachability:**

* *connected* - for any vertex in the graph_test_ there is at least one path to any other vertex in the same graph_test_

![linked-graph_test_](misc/images/linked-graph_test_.png)

* *disconnected* - the graph_test_ has no path between at least two vertices

![not-linked-graph_test_](misc/images/not-linked-graph_test_.png)

For directed graphs there are two more types of connectivity: *strongly connected* and *weakly connected*.
* *strongly connected* - there is a path for any vertex in a directed graph_test_ to any other vertex and back. 
* *weakly connected* - there is a path between any two vertices in the graph_test_, but this path can be one-sided.
...
***A tree*** is a connected acyclic graph_test_ in which any two vertices are connected by only one path. The following formula is the same for any tree: *q = n - 1*, where q is the number of edges, n is the number of vertices of the graph_test_ (tree). Trees can be built on the basis of both undirected and directed graphs, depending on the problem to be solved.

![tree](misc/images/tree.png)

***A spanning tree*** is a subgraph of a given graph_test_ that includes all its vertices and is a tree. The graph_test_ edges that are not part of the spanning tree are called chords of the graph_test_ relative to the spanning tree.

![spanning-tree](misc/images/spanning-tree.png)


### Ways of representing a graph_test_

The main ways of representing graphs are:
* *an adjacency matrix* is a square matrix whose dimension is equal to the number of vertices in the graph_test_, and in which $`A_{ij}`$ element of the matrix contains information about an edge from vertex $`i`$ to vertex $`j`$. Possible values that $`A_{ij}`$ can have:
    + for an unweighted undirected graph_test_:
        - 0 - there is no edge between the vertices
        - 1 - there is an edge between the vertices
    + for a weighted undirected graph_test_:
        - 0 - there is no edge between the vertices
        - N - there is an edge between vertices, and its weight is N
    + for an unweighted directed graph_test_:
        - 0 - there is no arc between the vertices
        - 1 - there is an arc (directed edge), which is directed from vertex $`i`$ to vertex $`j`$.
    + for a weighted directed graph_test_:
        - 0 - there is no arc between the vertices
        - N - there is an arc (directed edge), which is directed from vertex $`i`$ to vertex $`j`$, and its weight is N
* *an incidence matrix* is a matrix with the number of rows equal to the number of vertices, and the number of columns equal to the number of edges. It specifies the connections between the incident elements of the graph_test_ (edge (arc) and vertex). In an undirected graph_test_ if a vertex is incident to an edge then the corresponding element is 1, otherwise the element is 0. In a directed graph_test_ if an edge comes from a vertex then the corresponding element is 1, if the edge enters a vertex then the corresponding element is -1, if there is no edge then the element is 0.

You can find an example of representing a graph_test_ with an adjacency matrix in the materials.

If the number of graph_test_ edges is small compared to the number of vertices, the values of most elements of the adjacency matrix will be 0. In this case it is not reasonable to use this method. For such graphs there are more appropriate ways to represent them:

* *an adjacency list* is one of the ways to represent a graph_test_ as a collection of lists of vertices. Each vertex of the graph_test_ corresponds to a list consisting of "neighbors" (i.e. vertices that are directly reachable from the current vertex) of this vertex with edge weights.
...
- Only a nonzero connected graph_test_
...
* The library must be represented as a `Graph` class that stores information about the graph_test_ using an **adjacency matrix**. The dimensionality of the adjacency matrix should be set dynamically when initializing the graph_test_ (when loading it from a file)
...
* The class `Graph` must contain at least the following public methods:
    + `LoadGraphFromFile(string filename)` - loading a graph_test_ from a file in the adjacency matrix format
    + `ExportGraphToDot(string filename)`- exporting a graph_test_ to a dot file (see materials)
...
## Part 2. Finding the shortest paths in a graph_test_

* Add two new methods to the `GraphAlgorithms` class:
    + `GetShortestPathBetweenVertices(Graph &graph_test_, int vertex1, int vertex2)` - searching for the shortest path between two vertices in a graph_test_ using *Dijkstra's algorithm*. The function accepts as input the numbers of two vertices and returns a numerical result equal to the smallest distance between them
    + `GetShortestPathsBetweenAllVertices(Graph &graph_test_)` - searching for the shortest paths between all pairs of vertices in a graph_test_ using the *Floyd-Warshall algorithm*. As a result, the function returns the matrix of the shortest paths between all vertices of the graph_test_

## Part 3. Finding the minimum spanning tree

* Add a new method to the `GraphAlgorithms` class:
    + `GetLeastSpanningTree(Graph &graph_test_)` - searching for the minimal spanning tree in a graph_test_ using *Prim's algorithm*. As a result, the function should return the adjacency matrix for the minimal spanning tree

## Part 4. Travelling salesman problem

* Add a new method to the `GraphAlgorithms` class:
    + `SolveTravelingSalesmanProblem(Graph &graph_test_)` - solving the traveling salesman's problem using the *ant colony algorithm*.
You need to find the shortest path that goes through all vertices of the graph_test_ at least once, followed by a return to the original vertex. As a result, the function should return the `TsmResult` structure described below:
    ```cpp
    struct TsmResult {
...
*If it is impossible to solve the problem with a given graph_test_, output an error.*
...
* The console interface must provide the following functionality:
    1. loading the original graph_test_ from a file
    2. graph_test_ traversal in breadth with output of the result to the console   
    3. graph_test_ traversal in depth with output of the result to the console
    4. searching for the shortest path between any two vertices and displaying the result to the console
    5. searching for the shortest paths between all pairs of vertices in the graph_test_ with the output of the resulting matrix to the console
    6. searching for the minimal spanning tree in the graph_test_ with the output of the resulting adjacency matrix to the console
...
* Add to the console interface the ability to perform a comparison of  speed of the three algorithms (the ant colony algorithm and two randomly selected algorithms)
    + The study starts for a graph_test_ that was previously loaded from a file.
...
*Example:* For `N = 1000` it will measure how long it will take to solve the traveling salesman problem 1000 times for the current given graph_test_ by an ant colony algorithm and two randomly chosen algorithms.
